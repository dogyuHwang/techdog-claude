...
### Phase 0.5: Skill Injection (자동)
0. Scan `.tdc/learned-skills/*.md` for matching skills:
...
16. **If Ralph mode is active**: run the Ralph 검증 루프 (test → build → review → final verify) instead of single review pass
...
rm -f .tdc/context/.phase .tdc/context/.agent-status .tdc/context/.agent-events .tdc/context/.read_tokens .tdc/context/.compaction_done .tdc/context/.budget_warned .tdc/context/.ralph .tdc/context/.rate_limit
```
