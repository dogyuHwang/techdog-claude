...
| 가중 점수 | 판단 | 행동 |
|-----------|------|------|
| **4.0 이상** | 명확 | 질문 없이 바로 파이프라인 시작 |
| **2.5 ~ 3.9** | 부분적 모호 | 부족한 차원에 대해서만 질문 |
| **2.5 미만** | 모호 | 전체적 소크라틱 인터뷰 진행 |

### 소크라틱 인터뷰 (점수 2.5 미만 시)
...
18. **If Ralph mode is active**: run the Ralph 검증 루프 (test → build → review → final verify) instead of single review pass
...
18. Present a single final summary to the user (with token usage dashboard)
...
## Ralph Mode (끈질긴 검증 모드)

사용자가 `ralph:` 키워드로 시작하면 Ralph 모드가 활성화된다. 일반 모드보다 **훨씬 엄격한 검증 루프**를 실행한다.

### Ralph 모드 활성화

다음 패턴 중 하나로 활성화:
- `ralph: <작업 설명>` — 키워드로 활성화
- `/tdc ralph <spec.md>` — 서브커맨드로 활성화
...
Developer 구현 완료
    ↓
[Ralph Step 1] 테스트 실행 (있으면)
    ├→ FAIL → Developer 수정 (최대 3회)
    │         └→ 3회 실패 → Architect 에스컬레이션
    └→ PASS ↓
[Ralph Step 2] 빌드/타입체크 실행 (있으면)
    ├→ FAIL → Developer 수정 → Step 1로 돌아감
    └→ PASS ↓
[Ralph Step 3] Reviewer 코드 리뷰
...
Ralph 모드 시 `.tdc/context/.ralph` 파일 생성:
```bash
echo "RALPH=active" > .tdc/context/.ralph
echo "RETRY_COUNT=0" >> .tdc/context/.ralph
echo "VERIFY_PASS=0" >> .tdc/context/.ralph
echo "TOTAL_REGRESSIONS=0" >> .tdc/context/.ralph
```

Phase 4 완료 시 삭제:
```bash
rm -f .tdc/context/.ralph
```
...
rm -f .tdc/context/.phase .tdc/context/.agent-status .tdc/context/.agent-events .tdc/context/.read_tokens .tdc/context/.compaction_done .tdc/context/.budget_warned .tdc/context/.ralph .tdc/context/.rate_limit .tdc/context/notepad.md .tdc/context/.agent-tokens
```
...
- **Max 5 design-level regressions** (일반 모드) — then escalate to user. Ralph 모드에서는 무제한.
- **Ralph 모드**: `ralph:` 키워드 감지 시 활성화. `.tdc/context/.ralph` 파일 생성. 검증 루프 강화.
